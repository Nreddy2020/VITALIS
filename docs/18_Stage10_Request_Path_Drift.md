...
> **Scope correction, 2026-09-10: attaching per-manifest checks to traced services does not prove compatibility across their boundaries. service.version is a sender claim; equality with source is not artifact identity. Market exclusivity assertions below are unproven. The new boundary report is separate from the legacy source-manifest findings.** See [28](28_Request_Compatibility_Pilot.md) for step-by-step implementation, actual validation, limits and the comparison procedure.
