...
> **Organizational correction, 2026-09-10: AppDynamics is the primary monitoring product, with limited other request-flow tooling. The BMC/Prometheus/Splunk/GCL estate assumed below is not confirmed and must not drive design or a proposal. Actual incident access and AppDynamics coverage remain unknown. Only local pilot work is authorized.** See [28](28_Request_Compatibility_Pilot.md) for step-by-step implementation, actual validation, limits and the comparison procedure.
